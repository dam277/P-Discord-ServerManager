...
class AddFile(Add):
    ...
    @Command.register(name="file", description="Add a file to the server", parent="/add", file="File attachement which you want to add imported from your pc")
    async def execute(self, interaction: nextcord.Interaction):
        ...
        if self.file.size > self.settings["resources"]["max_file_size"]:
            await interaction.response.send_message("The file is too big to be send back to you (Maximum 25mo), do you still want to send this file ?", ephemeral=True)
            # TODO: Wait the confirmation view
