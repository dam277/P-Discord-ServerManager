...
class AddNote(Add):
    ...
    @Command.register(name="add_note", description="Add a note into a note list", parent="/add", title="Title of the note", text="Text of the note", note_list_name="Name of the note list")
    async def execute(self, interaction: nextcord.Interaction):
        ...
        if notes_result.get("objects") and len([note.title for note in notes_result.get("objects") if note.title == self.title]) > 0:
