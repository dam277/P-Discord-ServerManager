...
class PlaylistAutocomplete(Autocomplete):
    """ # PlaylistAutocomplete autocomplete class
    
    Description :
    ---
        Manage the PlaylistAutocomplete discord autocomplete
        
    Access :
    ---
        src.bots.server_manager.srvm_autocompletes.notes.PlaylistAutocomplete.py\n
    ...
    def __init__(self, current: str, guild_id: int):
        """ # PlaylistAutocomplete autocomplete constructor
        
        Description :
        ---
            Construct a PlaylistAutocomplete autocomplete object
            
        Access :
        ---
            src.bots.server_manager.srvm_autocompletes.notes.PlaylistAutocomplete.py\n
            PlaylistAutocomplete.__init__()
    ...
    async def execute(self, interaction: nextcord.Interaction):
        ...
        if server_id_result.get("value"):
            # Get the notelists
            notelists_result = await Playlist.get_playlists_by_server_id(server_id_result.get("value"))
