...
-- * Generation date: Mon Feb  5 16:12:44 2024 
...
create database DiscordManagerV5;
use DiscordManagerV5;
...
create table discordServer (
     id_discordServer bigint not null auto_increment,
     guildID bigint not null,
     name varchar(150) not null,
     constraint ID_discordServer_ID primary key (id_discordServer));

create table file (
     id_file bigint not null auto_increment,
     name varchar(255) not null,
     path varchar(500) not null,
     fk_discordServer bigint not null,
...
alter table file add constraint FKhave_FK
     foreign key (fk_discordServer)
     references discordServer (id_discordServer);
...
alter table server add constraint FKposess_FK
     foreign key (fk_discordServer)
     references discordServer (id_discordServer);
...
create unique index ID_discordServer_IND
     on discordServer (id_discordServer);

create unique index ID_file_IND
     on file (id_file);

create index FKhave_IND
     on file (fk_discordServer);
